    ...
    Column(modifier = Modifier.fillMaxWidth()) {
        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Text(item.categoryName, fontWeight = FontWeight.Medium, color = cs.onSurface, fontSize = 14.sp)
            }
            Text("Rs.${item.totalAmount / 100}", fontWeight = FontWeight.SemiBold, color = cs.onSurface, fontSize = 14.sp)
        }
